...
class PositionEmbedding(nn.Module):
    ...
    def __init__(self,
        ...
        self.num_embeddings = num_embeddings  # Độ dài của mỗi hàng dữ liệu
        self.embedding_dim = embedding_dim
        self.mode = mode

        # Khởi tạo trọng số là một tham số có thể học được
        if self.mode == self.MODE_EXPAND:
            self.weight = nn.Parameter(torch.Tensor(num_embeddings * 2 + 1, embedding_dim))
        else:
            self.weight = nn.Parameter(torch.Tensor(num_embeddings, embedding_dim))
        # Hàm này thường được sử dụng để khởi tạo trọng số, nhưng có thể được bỏ qua nếu muốn
    ...
    def forward(self, x):
        if self.mode == self.MODE_EXPAND:
            # Tính chỉ số indices để lựa chọn các vector embedding tương ứng từ trọng số
        ...
        # Kích thước batch và chiều dài chuỗi đầu vào
        batch_size, seq_len = x.size()[:2]
        embeddings = self.weight[:seq_len, :].view(1, seq_len, self.embedding_dim)
        if self.mode == self.MODE_ADD:
            # Phương pháp "add" - cộng thêm vector embedding vào đầu vào
            return x + embeddings
        if self.mode == self.MODE_CONCAT:
            # Phương pháp "concat" - nối vector embedding vào cuối đầu vào
